...
export default function PreviousResults() {
  ...
  async function createAndDownloadPdf() {
    axios.post('http://localhost:3000/report-pdf/download', { testName: test.title, name: user.name, isPassed: Number(test.passingScore) <= Number(result.score), correctAns: result.score, wrongAns: test.highestMarks - result.score })
      .then(() => axios.get('http://localhost:3000/report-pdf/fetch-pdf', { responseType: 'blob' }))
      .then((res) => {
  ...
  return <div>
      ...
      <div>
        ...
        {test._id && <table className="mt-10">
          <tbody>
            ...
            <tr>
              <td className="pr-4"><strong>Test Status:</strong> {test?.isReleased ? Number(test.passingScore) <= Number(result.score) ? "Passed" : "Failed" : "Test answers not released yet"}</td>
    ...
    {test?.isReleased && !loading && <>
      <button className="btn-tertiary mt-8 mx-auto block" onClick={createAndDownloadPdf}>Download Report</button>

      <h2 className="text-center mt-10">List Of Questions</h2>
      <div className="max-w-[500px] px-10 mx-auto">
        {test._id && test?.questions.map((question, index) => <Question
          key={question._id}
...
function Question({ question, answer }) {
  return <div className="mt-10 border-2">
    ...
    <p className="text-right">{question.answer === answer ? "correct answer" : answer === -1 ? "not answered" : "wrong answer"}</p>
    {question.options.map((option, index) =>
      <div key={option.id}
        className={`px-4 py-2 ${answer === option.id && option.id !== question.answer ? "bg-red-500" : ""} ${option.id === question.answer ? "bg-green-500" : ""} `}
      >
